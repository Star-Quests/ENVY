...
class JournalManager {
    ...
    // Show message instead of sell form
    notificationSystem.info('To sell, click on your active buy trade in the table below, click the eye icon to view details, then click "Edit Trade", expand Advanced Options, enter your Exit Price, and click "Update Trade"');
    
    // Reset back to buy after showing message
    setTimeout(() => {
        this.setTradeType('buy');
        document.getElementById('linkedBuyGroup').style.display = 'none';
        document.getElementById('partialCloseGroup').style.display = 'none';
        document.getElementById('sellInstructionBanner').style.display = 'none';
        document.getElementById('priceLabel').textContent = 'Entry Price (USD)';
        document.getElementById('exitPriceLabel').textContent = 'Exit Price (Optional)';
        document.getElementById('priceInput').placeholder = '0.00';
    }, 100);
    ...
    // Calculate P/L using the correct formula
    ...
    // 📦 UPDATE HOLDINGS
    console.log('🔴 HOLDINGS DEBUG - buyTrade:', buyTrade);
    console.log('🔴 HOLDINGS DEBUG - buyTrade.asset_symbol:', buyTrade.asset_symbol);
    console.log('🔴 HOLDINGS DEBUG - amount to subtract:', amount);
    
                    const { data: holding, error: holdingError } = await supabase
                    .from('holdings')
                    .select('*')
                    .eq('user_id', this.user.id)
                    .eq('asset_symbol', buyTrade.asset_symbol)
                    .single();
    
    
        
    if (holding) {
                const newAmount = holding.total_amount - amount;
        
        if (newAmount <= 0.00000001) {
                        const { error: deleteError } = await supabase.from('holdings').delete().eq('id', holding.id);
        } else {
                        const { error: updateError } = await supabase.from('holdings').update({ total_amount: newAmount }).eq('id', holding.id);
        }
    } else {
                console.log('⚠️ No holding found for:', buyTrade.asset_symbol);
    }
}
